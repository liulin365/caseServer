...
import com.caseServer.UI.AutoLogger;
import com.caseServer.UI.FFDriver;
import com.caseServer.UI.GoogleDriver;
import com.caseServer.UI.IEDriver;
...
//		}
		...
		public void login(String username,String password) {
			try {
				visitWeb("http://192.168.12.241:8090/caseServer/login.action");
				halt("5");
				input("//*[@id='username']", username);
				input("//input[@id='password']", password);
				click("//*[@id='loginBtn']");
			} catch (Exception e) {
				AutoLogger.log.info("登录失败！");
			}
		}
		 ...
		 * 断言页面源码中不包含某个内容
		 * 
		 * @param notarget 页面中预期不包含的内容
		 */
			public void assertPageNotContains(String notarget) {
				String pageContentno = driver.getPageSource();
				if (!pageContentno.contains(notarget)) {
					AutoLogger.log.info("测试成功！");
					webExcel.writeCell(line, 10, "PASS");
				} else {
					AutoLogger.log.info("测试失败！");
			...
			/**
			 * 提取回收登记中状态为未回收的第一行数据的住院号
			 * 
			 * @param key 存入map中的key值，可以随意取字符串
			 */	
			public void saveParam(String key) {
				String value;
				try {
					value = getJs("document.getElementsByClassName(\"datagrid-cell datagrid-cell-c2-zyh\")[0].innerText");					
			 ...
			 * 从map中提取住院号，并输入
			 * 
			 * @param zyhxpath 输入住院号的xpath
			 * @param zyhkey，住院号在map中的key值，要与saveParam中的key一致
			 */	
			public void insertZyh(String zyhxpath,String zyhkey) {
				try {
					input(zyhxpath,paramMap.get(zyhkey));
					webExcel.writeCell(line, 10, "PASS");
				} catch (Exception e) {
					AutoLogger.log.error("输入参数失败");
					AutoLogger.log.error(e,e.fillInStackTrace());
					webExcel.writeFailCell(line, 10, "FAIL");
				}
			}
...
//			实现鼠标双击
			public void doubleClick(String xpath) {
				try {
					...
					WebElement delement = driver.findElement(By.xpath(xpath));
					actions.doubleClick(delement).perform();
					webExcel.writeCell(line, 10, "PASS");
				} catch (Exception e) {
					AutoLogger.log.error(e,e.fillInStackTrace());
					e.printStackTrace();
					saveScrShot("doubleclick");
					webExcel.writeFailCell(line, 10, "FAIL");
				}
			}
